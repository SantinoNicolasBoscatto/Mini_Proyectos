...
namespace NotesApp.Repository
{
    public class ToDoTaskRepository : IToDoTaskRepository
    {
        ...
        public async Task<List<ToDoTask>> GetPerUserNote(string userId, int noteId, bool orderByStatus = false, bool orderByPriority = true, bool asc = true)
        {
        ...
        public async Task UpdateToDoState(string userId)
        {
            var toDoList = await _dbContext.ToDoTasks
                .Where(x => x.Note.IdentityUserId == userId).AsNoTracking().ToListAsync();
            foreach (var toDo in toDoList)
            {
                if(toDo.DateLimit < DateTime.Now && toDo.StatusId == 1)
                {
                    toDo.StatusId = 2;
                    _dbContext.ToDoTasks.Update(toDo);
                }

                if (toDo.DateLimit >= DateTime.Now && toDo.StatusId == 2)
                {
                    toDo.StatusId = 1;
                    _dbContext.ToDoTasks.Update(toDo);
                }
